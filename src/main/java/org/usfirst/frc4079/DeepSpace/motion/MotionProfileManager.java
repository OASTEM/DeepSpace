 ...
 */
package org.usfirst.frc4079.DeepSpace.motion;
...
public class MotionProfileManager {
	 ...
	 */
	public MotionProfileManager(TalonSRX talon, double[][] profile) {
		_talon = talon;
		/*
	...
	*/
	...
	private void startFilling(double[][] profile, int totalCnt) {
		...
		for (int i = 0; i < totalCnt; ++i) {
			double positionNativeUnits = profile[i][0];
			double velocityNativeUnits = profile[i][1];
			/* for each point, fill our structure and pass it to API */
			point.position = positionNativeUnits;
			point.velocity = velocityNativeUnits;
