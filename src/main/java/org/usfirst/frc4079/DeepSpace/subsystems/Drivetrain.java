...
public class Drivetrain extends Subsystem implements PIDOutput {
    ...
    public Drivetrain() {
        ...
        leftMaster = new TalonSRX(1); // 0 for 2019, 1 for 2018
        leftSlave = new TalonSRX(2);
        rightMaster = new TalonSRX(3); // 1 for 2019, 3 for 2018
        rightSlave = new TalonSRX(0);  // 3 for 2019, 0 for 2018
	...
	public synchronized void initTalons(){
		...
		leftMaster.configClosedloopRamp(Constants.kStraightClosedLoopRampRate, Constants.kTimeoutMs);
		leftMaster.configOpenloopRamp(Constants.kStraightOpenLoopRampRate, Constants.kTimeoutMs);
		rightMaster.configClosedloopRamp(Constants.kStraightClosedLoopRampRate, Constants.kTimeoutMs);
		rightMaster.configOpenloopRamp(Constants.kStraightOpenLoopRampRate, Constants.kTimeoutMs);
		leftSlave.configClosedloopRamp(Constants.kStraightClosedLoopRampRate, Constants.kTimeoutMs);
		leftSlave.configOpenloopRamp(Constants.kStraightOpenLoopRampRate, Constants.kTimeoutMs);
		rightSlave.configClosedloopRamp(Constants.kStraightClosedLoopRampRate, Constants.kTimeoutMs);
		rightSlave.configOpenloopRamp(Constants.kStraightOpenLoopRampRate, Constants.kTimeoutMs);
    ...
    public void configureForTurn() {
		...
		leftMaster.selectProfileSlot(Constants.kSlotTurn, 0);
		 ...
		 leftMaster.configClosedloopRamp(Constants.kTurnClosedLoopRampRate, Constants.kTimeoutMs);
		 leftMaster.configOpenloopRamp(Constants.kTurnOpenLoopRampRate, Constants.kTimeoutMs);

		 rightMaster.configClosedloopRamp(Constants.kTurnClosedLoopRampRate, Constants.kTimeoutMs);
		 rightMaster.configOpenloopRamp(Constants.kTurnOpenLoopRampRate, Constants.kTimeoutMs);

		 leftSlave.configClosedloopRamp(Constants.kTurnClosedLoopRampRate, Constants.kTimeoutMs);
		 leftSlave.configOpenloopRamp(Constants.kTurnOpenLoopRampRate, Constants.kTimeoutMs);
		 
		 rightSlave.configClosedloopRamp(Constants.kTurnClosedLoopRampRate, Constants.kTimeoutMs);
		 rightSlave.configOpenloopRamp(Constants.kTurnOpenLoopRampRate, Constants.kTimeoutMs);
	}
	 
	public double convertAngle(double goal) {
		if (Math.abs(goal) < Constants.angleKey[0]) {
			return 0;
		}
		
		for (int i = 1; i < Constants.angleKey.length; i++) {			
			double angleAdjust = (Constants.angleVal[i] - Constants.angleVal[i-1])*(Math.abs(goal) - Constants.angleKey[i-1])/(Constants.angleKey[i]-Constants.angleKey[i-1]); 
			
			if (Math.abs(goal) < Constants.angleKey[i]) {
				if (goal < 0) {
					return -Constants.angleVal[i-1] - angleAdjust;
				}
